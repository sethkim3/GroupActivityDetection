...
def learn_activities(features_df, model_type):
    """
    Train a model to classify activities based on engineered features and use cross-validation for accuracy.
    :param features_df: A dataframe generated by generate_samples and engineer_features.
    :param model_type: Model to use for classification. Currently supports NaiveBayes, RandomForest, GradientBoost
    :return: A trained model, cross_val score, list of selected top-5 features.
    """

    # prepare the data
    features_df = features_df.dropna()
    X = features_df.drop(columns=['sample_id','label'])
    y = features_df['label']
    ...
    print('5-fold Cross Validation score for ' + model_type + ' model: ')
    scores = cross_val_score(model, X, y, cv=5)
    print("\t Accuracy: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))

    # fit the model exactly to the training data
    model.fit(X, y)

    features = []
    if (model_type in ['RandomForest', 'GradientBoost']):
        ...
        columns = features_df.drop(columns=['sample_id','label']).columns
...
def predict_activities(features_df, model, activities):
    """
    ...
    :return: Classifications of the input data, and accuracies, precisions, recalls for the classes.
    """
    ...
    for i in range(len(activities)):
        for j in range(len(y)):
            if (i == y[j]):
                y_test_class.append(y[j])
                y_pred_class.append(y_pred[j])
        if (y_test_class != []):
